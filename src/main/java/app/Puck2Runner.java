...
public class Puck2Runner {
    ...
    //parse et ajoute tous les fichiers � la liste de compilation 
    private void loadProgram(String path) throws IOException {
        File f = new File(path); 
        if (f.isDirectory()) { //si f est un r�pertoire
            for (File innerFile: f.listFiles()) {
                loadProgram(innerFile.getAbsolutePath()); //r�cursivement ? on charge chaque fichier du projet
            }
        } else if (getFileExtension(path).equals("java")) { //si c'est un fichier java
            program.addSourceFile(path); //parse le fichier et ajoute to compilation unit � la liste de compilation unit
        }
    }
